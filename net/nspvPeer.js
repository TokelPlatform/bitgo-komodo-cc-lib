...
Peer.prototype._registerListeners = function() {
  ...
  this.on('nSPV', (buf) => {
    // forward nSPV response to the function which sent getNSPV with the request id 
    let resp = nspvResp.decode(buf, undefined, undefined);
...
Peer.prototype._formatCommand = function(command, payload) {
  ...
  if (command == 'nSPV' && payload[0] == NSPVMSGS.NSPV_ERRORRESP)  {
    let resp = nspvResp.decode(payload, undefined, undefined);
...
// nspv get txids v2 (with begin/end height params)
Peer.prototype.nspvGetTxidsV2 = function(address, isCC, beginHeight, endHeight, opts, cb) {
  if (typeof opts === 'function') {
    cb = opts
    opts = {}
  }
  ...
  var onNspvResp = (resp) => {
    if (timeout) clearTimeout(timeout)
    if (resp && resp?.respCode === NSPVMSGS.NSPV_ERRORRESP) {
      cb(new NspvError("nspv get txids remote error: " + resp?.errDesc, NSPVMSGS.NSPV_TXIDS_V2)); 
      return;
    }
    cb(null, resp, this)
  }
  ...
  let nspvReqTxids = {
    reqCode: NSPVMSGS.NSPV_TXIDS_V2,
    requestId: requestId,
    coinaddr: address,
    CCflag: isCC ? 1 : 0,
  ...
  if (!opts.timeout) return
  timeout = setTimeout(() => {
    logerror(`getnSPV NSPV_TXIDSRESP_V2 timed out: ${opts.timeout} ms`)
    var error = new NspvError('NSPV request timed out', NSPVMSGS.NSPV_TXIDS_V2)
    error.timeout = true
    cb(error, null, this)
  }, opts.timeout)
}
...
Peer.prototype.nspvTxProof = function(_txid, vout, height, opts, cb) {
  ...
  timeout = setTimeout(() => {
    ...
    // exception don't disconnect if rate limiter did not respond for this msg
    // error.timeout = true
    error.rateTimeout = true  // special flag to indicate time out possbly due to rate limiter
    cb(error, null, this)
  }, opts.timeout)
}
